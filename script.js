...
d3.json("GDP-data.json")
  .then(sura => {
    ...
    const years = sura.data.map((item) => {
      let quarter;
    ...
    const yearsDate = sura.data.map((item) => new Date(item[0]));
    ...
    const GDP = sura.data.map((item) => item[1]);
    ...
    d3.select('svg')
      ...
      .attr('data-date', function (d, i) {
        return sura.data[i][0];
      })
      .attr('data-gdp', function (d, i) {
        return sura.data[i][1];
      })
      .attr('class', 'bar')
      .attr('x', function (d, i) {
